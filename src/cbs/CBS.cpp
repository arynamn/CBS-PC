...
void CBS::findConflicts(CBSNode &curr, int a1, int a2)
{
	...
	for (auto cons : search_engines[0]->instance.temporal_cons[a1 * num_of_agents + a2])
	{
		...
		{
			cout << "Temporal conflict between " << a1 << "(" << from_landmark << ")"
				 << " and " << a2 << "(" << to_landmark << ")" << endl;
	...
	for (auto cons : search_engines[0]->instance.temporal_cons[a2 * num_of_agents + a1])
	{
		...
		{
			cout << "Temporal conflict between " << a2 << "(" << from_landmark << ")"
				 << " and " << a1 << "(" << to_landmark << ")" << endl;
			shared_ptr<Conflict> conflict(new Conflict());
	...
	for (size_t timestep = 0; timestep < min_path_length; timestep++)
	{
		...
		else if (timestep < min_path_length - 1 && loc1 == paths[a2]->at(timestep + 1).location && loc2 == paths[a1]->at(timestep + 1).location)
		{
			shared_ptr<Conflict> conflict(new Conflict());
...
bool CBS::findPathForSingleAgent(CBSNode *node, int ag, int lowerbound)
{
	...
	if (!new_path.empty())
	{
		...
		if (screen >= 2)
		{
			...
			for (int j = 0; j < (int)new_path.timestamps.size(); j++)
			{
	...
	for (int a1 = 0; a1 < num_of_agents; a1++)
	{
		for (int a2 = 0; a2 < num_of_agents; a2++)
		{
			...
			for (size_t timestep = 0; timestep < min_path_length; timestep++)
			{
				...
				{
					cout << "Agents " << a1 << " and " << a2 << " collides at (" << loc1 << "-->" << loc2 << ") at timestep " << timestep << endl;
					return false;
				}
			}
			if (paths[a1]->size() != paths[a2]->size())
