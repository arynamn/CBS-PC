...
inline bool PBS::is_task_a_final_one(int task)
{
	...
	return i == (int)search_engines[agent]->goal_location.size() - 1;
}
...
PBS::PBS(const Instance &instance, int screen) : CBS(instance, false, heuristics_type::ZERO, screen)
{
	...
	for (int i = 0; i < num_of_agents; i++)
	{
		...
		for (int j = 0; j < (int)search_engines[i]->goal_location.size(); j++)
		{
	...
	for (int i = 0; i < num_of_agents; i++)
	{
		for (int j = 1; j < (int)search_engines[i]->goal_location.size(); j++)
		{
			initial_priorities.push_back({task2id({i, j - 1}), task2id({i, j}), -1, -1, constraint_type::GPRIORITY});
		}
	}
	for (int i = 0; i < (int)instance.temporal_cons.size(); i++)
	{
...
bool PBS::topological_sort(vector<vector<int>> &adj_list, vector<int> &planning_order)
{
	...
	assert((int)planning_order.size() == num_of_tasks);
	return true;
}
...
void PBS::build_ct(ConstraintTable &ct, int task_id, vector<vector<int>> adj_list_r)
{
	...
	for (int i = 0; i < num_of_tasks; i++)
	{
		if (high_prio_agents.find(i) != high_prio_agents.end())
		{
			...
			bool wait_at_goal = task == (int)search_engines[agent]->goal_location.size() - 1;
...
bool PBS::generateChild(CBSNode *node, CBSNode *parent)
{
	...
	for (auto i : planning_order)
	{
		...
		if (paths[i]->empty())
		{
			...
			{
				reuse_old_path = true;
				for (int j = 0; j + 1 < (int)copy[i]->size(); j++)
				{
	...
	for (int i = 0; i < num_of_agents; i++)
	{
		...
		for (int j = 0; j < (int)search_engines[i]->goal_location.size(); j++)
		{
			...
			if (j + 1 < (int)search_engines[i]->goal_location.size())
			{
...
bool PBS::solve(double time_limit, int cost_lowerbound, int cost_upperbound)
{
	...
	while (!dfs_stack.empty() && !solution_found)
	{
		...
		if (runtime > time_limit || (int)num_HL_expanded > node_limit)
		{ // time/node out
			solution_cost = -1;
			solution_found = false;
			break;
...
void PBS::join_paths()
{
	...
	for (int i = 0; i < num_of_agents; i++)
	{
		for (int j = 0; j < (int)search_engines[i]->goal_location.size(); j++)
		{
			...
			assert((int)joined_paths[i].size() - 1 == paths[task_id]->begin_time);
			for (int k = 1; k < (int)paths[task_id]->size(); k++)
			{
