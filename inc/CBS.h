...
protected:
	...
	void copyConflicts(
		const list<shared_ptr<Conflict>> &conflicts,
		list<shared_ptr<Conflict>> &copy, 
		const list<int> &excluded_agent) const;
...
public:
	...
	void setRectangleReasoning(bool r) {
		rectangle_helper.use_rectangle_reasoning = r;
		heuristic_helper->rectangle_reasoning = r;
	}
	void setCorridorReasoning(bool c) {
		corridor_helper.use_corridor_reasoning = c;
	...
	void setMutexReasoning(mutex_strategy m) {
		mutex_helper.strategy = m;
